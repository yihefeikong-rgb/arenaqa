...
export function AnswerColumn({ model }: Props) {
  ...
  const answer = useChatStore((s) => s.answers[model]);
  ...
  return (
    <div className={`bg-white dark:bg-gray-800 rounded-xl border overflow-hidden transition-colors ${isStreaming ? "border-indigo-300 dark:border-indigo-600 animate-pulse-border" : "border-gray-200 dark:border-gray-700"}`}>
      {/* Header */}
      ...
      {(answer.status === "done" || hasContent) && (
        <div className="px-4 py-2 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            <button
              className={`px-2 py-1 rounded-md text-[11px] border transition-colors ${feedback === "like" ? "bg-green-50 border-green-200 text-green-600" : "border-gray-200 dark:border-gray-600 text-gray-400 hover:text-green-500 hover:border-green-200"}`}
            ...
            <button
              className={`px-2 py-1 rounded-md text-[11px] border transition-colors ${feedback === "dislike" ? "bg-red-50 border-red-200 text-red-500" : "border-gray-200 dark:border-gray-600 text-gray-400 hover:text-red-500 hover:border-red-200"}`}
          ...
          <div className="flex items-center gap-1.5">
            ...
            {hasContent && (
              <button
                className="px-2 py-1 rounded-md text-[11px] border border-gray-200 dark:border-gray-600 text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors"
      ...
      {/* 点踩反馈弹窗 */}
      {showDislikeDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 dark:bg-black/50" onClick={() => setShowDislikeDialog(false)}>
          <div className="bg-white dark:bg-gray-800 rounded-xl p-5 max-w-sm w-full mx-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-1">反馈原因</h4>
            <p className="text-xs text-gray-500 mb-3">选择你认为存在的问题（可多选）</p>
            <div className="flex flex-wrap gap-1.5 mb-3">
              {DISLIKE_TAGS.map((tag) => (
                <button key={tag}
                  className={`px-2.5 py-1 rounded-full text-xs border transition-colors ${dislikeTags.includes(tag) ? "bg-red-50 border-red-300 text-red-600" : "border-gray-200 dark:border-gray-600 text-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700/50"}`}
            ...
            <textarea className="w-full px-2.5 py-1.5 border border-gray-200 dark:border-gray-600 rounded-lg text-xs resize-none bg-gray-50 dark:bg-gray-700 dark:text-gray-200 focus:outline-none focus:border-indigo-400 mb-3" rows={2}
            ...
            <div className="flex gap-2 justify-end">
              <button onClick={() => { setShowDislikeDialog(false); setDislikeTags([]); setDislikeComment(""); }}
                className="px-3 py-1.5 rounded-lg text-xs border border-gray-200 dark:border-gray-600 text-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">取消</button>
