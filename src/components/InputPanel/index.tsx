...
export function InputPanel() {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 pt-4 pb-3">
        <div className="flex items-center gap-2.5">
          ...
          <div className="min-w-0">
            <h1 className="text-[15px] font-semibold text-[var(--color-text-primary)] leading-tight truncate">
              AI 问答竞技场
            </h1>
            <p className="text-[var(--font-caption)] text-[var(--color-text-disabled)] leading-tight mt-0.5">
              多模型并发对比
            </p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-5">
        ...
        <div role="group" aria-label="选择模型">
          <label className="flex items-center gap-1.5 mb-2.5">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-[var(--color-text-disabled)]">
              <rect x="3" y="3" width="18" height="18" rx="2" />
              <line x1="3" y1="9" x2="21" y2="9" />
              <line x1="9" y1="21" x2="9" y2="9" />
            </svg>
            <span className="text-[var(--font-caption)] font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider">
              模型
            </span>
            <span className="ml-auto text-[var(--font-caption)] text-[var(--color-text-disabled)] tabular-nums">
        ...
        <div>
          ...
          <textarea
            ...
            className="
              ...
              bg-[var(--color-card)] p-3
              text-[var(--font-body)] text-[var(--color-text-primary)]
              placeholder:text-[var(--color-text-disabled)]
              focus:outline-none focus:border-[var(--color-primary)] focus:ring-1 focus:ring-[var(--color-primary-glow)]
              transition-all duration-150 ease-out
              disabled:opacity-40
            "
            rows={4}
          />
          <p id="prompt-hint" className="mt-1.5 text-[var(--font-caption)] text-[var(--color-text-disabled)]">
      ...
      {/* 底部固定区 */}
      <div className="px-4 pb-4 pt-2 space-y-3 border-t border-[var(--color-divider)]">
        {/* 进度条 */}
        {isBusy && (
          <div className="space-y-1.5" role="progressbar" aria-valuenow={progress} aria-valuemin={0} aria-valuemax={100}>
            <div className="flex justify-between text-[var(--font-caption)]">
              <span className="text-[var(--color-text-secondary)]">
                {status === 'streaming' ? '回答中...' :
                 status === 'judging' ? '裁判评分...' : '融合生成...'}
              </span>
