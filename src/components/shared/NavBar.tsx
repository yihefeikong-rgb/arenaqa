...
export function NavBar() {
  ...
  return (
    <header className="h-12 border-b border-[var(--color-border)] bg-[var(--color-surface)] flex items-center justify-between px-5 shrink-0">
      {/* 左侧品牌 */}
      <div className="flex items-center gap-2.5">
        ...
        <span className="text-[var(--font-body)] font-semibold text-[var(--color-text-primary)] tracking-tight">
          ArenaQA
        </span>
        <span className="hidden md:inline text-[var(--font-caption)] text-[var(--color-text-disabled)] ml-0.5">
          AI 问答竞技场
        </span>
      </div>
      ...
      <nav className="flex items-center gap-1.5" aria-label="顶部导航">
        ...
        {(status !== 'idle') && (
          <button
            onClick={reset}
            className="h-7 px-2.5 rounded-[var(--radius-sm)] text-[var(--font-caption)] font-medium
                       text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]
                       hover:bg-[var(--color-bg)] transition-all duration-150 flex items-center gap-1.5"
            aria-label="新建问答"
          >
