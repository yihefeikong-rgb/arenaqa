...
export function useChat() {
  ...
  const extendBody = useCallback(
    (prompt: string, models: string[]) => {
      ...
      if (nimKey) {
        models.forEach((m) => { if (m.startsWith("nim-")) CUSTOM_KEYS[m] = nimKey; });
      }
      ...
      return { apiKeys: { ...apiKeys, ...CUSTOM_KEYS }, modelConfigs, customModels, judgeConfig, conversationId: conversationId ?? undefined, round: currentRound };
    },
    []
  );
  ...
  const sendChat = useCallback(
    async (prompt: string, models: string[]) => {
      ...
      // 如果是首次提问（无 conversationId），重置答案区；追问保留历史
      const isFollowUp = !!st.conversationId;
      if (!isFollowUp) {
        st.reset();
      } else {
        // 追问：只清空答案/评分/融合，保留 conversationId 和 round
      ...
      try {
        ...
        eventSource.addEventListener("done", (e: Event) => {
          const me = e as MessageEvent;
          const data = JSON.parse(me.data);
          flushAll((model, text) => { useChatStore.getState().appendChunk(model, text); });
        ...
        eventSource.addEventListener("error", (e: Event) => {
          ...
          } catch { console.warn('[useChat] SSE error parse failed (may be internal)'); }
        });
