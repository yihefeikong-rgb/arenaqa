...
set "KILLED="
for /f "usebackq tokens=5" %%a in (`netstat -ano ^| findstr ":3000" ^| findstr "LISTEN"`) do (
    if not "%%a"=="" (
...
for /l %%i in (1,1,30) do (
    >nul 2>&1 powershell -Command "try { $r = Invoke-WebRequest -Uri 'http://localhost:3000' -UseBasicParsing -TimeoutSec 2; if ($r.StatusCode -eq 200) { exit 0 } else { exit 1 } } catch { exit 1 }"
    if !errorlevel! equ 0 (
        set READY=1
        goto SERVER_READY
    )
